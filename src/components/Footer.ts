/**
 * Footer Component
 * Shows fixed at bottom of hero, hidden on scroll, reappears at page bottom
 */

export interface SocialLink {
    type: 'discord' | 'roblox' | 'custom'
    label: string
    href: string
    icon?: string
}
...
// SVG Icons
const ICONS: Record<SocialLink['type'], string> = {
...
function createSocialButtons(socialLinks: ReadonlyArray<SocialLink>): HTMLElement {
    ...
    socialLinks.forEach(link => {
        ...
        if (link.type === 'discord') {
            bgColor = 'bg-black/20 hover:bg-[#5865F2]'
        } else if (link.type === 'roblox') {
            bgColor = 'bg-black/20 hover:bg-[#E2231A]'
        }

        btn.className = `
            flex items-center justify-center w-10 h-10
            ${bgColor} text-gray-300 hover:text-white
...
export function createHeroFooter(config: FooterConfig): HTMLElement {
    const { companyName, year = new Date().getFullYear(), socialLinks = [] } = config
    ...
    if (socialLinks.length > 0) {
        container.appendChild(createSocialButtons(socialLinks))
    }
